...
By default this creates a cluster labeled `demo-dev-cluster` in region `us-mia` with one node pool of 2x `g6-standard-2` nodes running Kubernetes `1.31`. Override any of `region`, `k8s_version`, `node_type`, `node_count`, or `cluster_label` via `-var` flags, a `*.tfvars` file, or environment variables (`TF_VAR_<name>`) to customize.
